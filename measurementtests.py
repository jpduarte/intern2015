        ...
        if state==1:
          ...
          if (currentdevice != ",".join(header[0:(self.device_tags.index('temperature')+2)])):
            ...
            if not namedevice in self.devices:
              ...
              self.devices[namedevice]  = {'technology':header[self.device_tags.index('technology')+1],'wafer':header[self.device_tags.index('wafer')+1],'site':header[self.device_tags.index('site')+1],'macro':header[self.device_tags.index('macro')+1],'device':header[self.device_tags.index('device')+1]} 
